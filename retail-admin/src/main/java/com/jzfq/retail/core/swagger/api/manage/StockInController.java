...
public class StockInController {
    ...
    @ResponseBody
    public ResponseEntity<TouchResponseModel> allList(@RequestBody PurchaseCollectGoodsReq search) {
        CommonValid.emptyStringToNull(search);
        try {
            List<StockInRes> result = stockInService.getAllList(search);
    ...
    @ResponseBody
    public ResponseEntity<TouchResponseModel> list(@Valid @RequestBody PageReq<PurchaseCollectGoodsReq> req) {
        try {
            ListResultRes result = stockInService.getList(req.getPage(), req.getPageSize(), req.getSearch());
            return TouchApiResponse.success(result,"操作成功");
        } catch (TouchCodeException te) {
            log.error("请求返回异常:{}", te.getMessage());
            return TouchApiResponse.failed(te.getTouchApiCode().getCode(), te.getTouchApiCode().getMsg() + te.getExtendMsg());
    ...
    @ResponseBody
    public ResponseEntity<TouchResponseModel> create(@Valid @RequestBody PurchaseCollectGoods req) {
        try {
            stockInService.create(req);
    ...
    @ResponseBody
    public ResponseEntity<TouchResponseModel> update(@Valid @RequestBody PurchaseCollectGoods req) {
        try {
            stockInService.update(req);
            return TouchApiResponse.success("操作成功");
        } catch (TouchCodeException te) {
            log.error("请求返回异常:{}", te.getMessage());
            return TouchApiResponse.failed(te.getTouchApiCode().getCode(), te.getTouchApiCode().getMsg() + te.getExtendMsg());
    ...
    public ResponseEntity<TouchResponseModel> getSellerById(@PathVariable("id") Integer id) {
        try {
            PurchaseCollectGoods entity = stockInService.getEntityById(id);
